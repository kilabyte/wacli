...
func (cli *Client) decryptMsgSecret(ctx context.Context, msg *events.Message, useCase MsgSecretType, encrypted messageEncryptedSecret, origMsgKey *waCommon.MessageKey) ([]byte, error) {
	...
	// Fast path: derive the key from the identities exactly as they appear in the new event.
	...
	// The message secret key (and AEAD additional data) is derived from the JID *strings* of
	// both the original message sender and the modification sender. While WhatsApp migrates to
	// LIDs, the realm (@s.whatsapp.net vs @lid) of either identity in the new event can differ
	// from the realm the encrypting client used to derive the key. Retry across the alternate
	// realms of both identities: the original sender from the message key, the sender stored
	...
	for _, modSender := range modSenders {
		for _, sender := range origSenders {
			if modSender == msg.Info.Sender.ToNonAD() && sender == origSender.ToNonAD() {
				continue // identical to the fast path attempted above
			}
...
// altRealmJIDs returns the given JIDs together with their PN<->LID counterparts, normalised to
// non-AD form and de-duplicated (empty JIDs are dropped). It is used to retry message secret
// decryption when an identity is addressed in a different realm (@s.whatsapp.net vs @lid) than
// the one the encrypting client used to derive the key. The first element for each input JID is
// the JID itself, so callers that try candidates in order attempt the original realm first.
func (cli *Client) altRealmJIDs(ctx context.Context, jids ...types.JID) []types.JID {
	out := make([]types.JID, 0, len(jids)*2)
	seen := make(map[types.JID]struct{}, len(jids)*2)
	...
	for _, jid := range jids {
		...
		if cli.Store != nil && cli.Store.LIDs != nil {
			if alt, err := cli.Store.GetAltJID(ctx, jid.ToNonAD()); err == nil {
				add(alt)
			}
		}
	}
